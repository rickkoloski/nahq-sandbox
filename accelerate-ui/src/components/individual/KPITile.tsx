...
export function KPITile({ icon: Icon, label, value, sub, color = '#00A3E0', badge, tooltip }: KPITileProps) {
  const [tipOpen, setTipOpen] = useState(false)

  return (
    <div className="bg-white rounded-xl border border-gray-200 px-5 py-5 shadow-sm">
      <div className="flex items-center gap-2 mb-3">
        <div aria-hidden="true" className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0" style={{ backgroundColor: `${color}15` }}>
        ...
        {tooltip && (
          <div className="ml-auto relative flex-shrink-0">
            <button type="button" aria-label={`More information: ${tooltip}`} aria-expanded={tipOpen}
            ...
            {tipOpen && (
              <div role="tooltip" className="absolute right-0 top-6 z-30 w-52 bg-gray-900 text-white text-[11px] leading-relaxed rounded-lg px-3 py-2 shadow-lg pointer-events-none">
                {tooltip}
              </div>
            )}
          </div>
      ...
      <div className="flex items-baseline gap-2">
        ...
        {badge && (
          <span className="text-[10px] font-bold px-2 py-0.5 rounded-full" style={{ backgroundColor: badge.bg, color: badge.text }}>
            {badge.label}
          </span>
        )}
      </div>
