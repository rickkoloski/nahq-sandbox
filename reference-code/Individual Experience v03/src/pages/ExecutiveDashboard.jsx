...
import { Filter, Target, TrendingUp, ChevronDown, ChevronRight } from 'lucide-react';
...
const TABS = [
  ...
  { id: 'progress', label: 'Pulse Survey Results', icon: TrendingUp },
];
...
export default function ExecutiveDashboard() {
  ...
  const handleAiChat = (context) => { setAiChatContext(context); setAiChatOpen(true); };
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FB]">
       ...
       <div className="bg-white border-b border-gray-200">
         <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
           ...
           <nav className="flex items-center gap-1.5 text-xs text-gray-400 py-3">
             ...
             <span className="text-[#3D3D3D] font-semibold">Organizational Dashboard</span>
           </nav>

           <div className="flex items-center justify-between py-4">
              <div>
                <h1 className="text-xl font-bold text-[#3D3D3D]">Organizational Dashboard</h1>
                <p className="text-xs text-gray-500 mt-0.5">Workforce capability · Assessment results · Development insights</p>
              </div>
            </div>
          ...
          <AnimatePresence mode="wait">
            {activeTab === 'explorer' && (
              <motion.div key="explorer" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.18 }}>
                {/* Strategic Summary Bar — always visible at top */}
                <StrategicSummaryBar onChatOpen={handleAiChat} />
            ...
            {activeTab === 'progress' && (
              <motion.div key="progress" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.18 }}>
                <ReassessmentProgressCard data={currentData} filters={filters} onChatOpen={handleAiChat} />
              </motion.div>
            )}
          </AnimatePresence>
        </div>
      ...
      <AnimatePresence>
        {aiChatOpen && (
          <ExecutiveAIChat isOpen={aiChatOpen} onClose={() => { setAiChatOpen(false); setAiChatContext(null); }} context={aiChatContext} data={currentData} />
        )}
      </AnimatePresence>
    </div>
  );
