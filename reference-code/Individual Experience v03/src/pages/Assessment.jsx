...
export default function Assessment() {
  ...
  const canProceed = !!currentAnswer.grouping;
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-white via-gray-50/50 to-white">
        ...
        <main id="main-content" className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <Breadcrumb items={[{ label: 'Take Your Assessment' }]} />
          ...
          >
            <h1 className="text-2xl font-bold text-[#3D3D3D] mb-6">
              NAHQ Professional Assessment Terms of Use
            </h1>

            {/* Scrollable terms — keyboard-reachable, labelled region */}
            <div
              role="region"
              aria-label="Terms of use content — scroll to read"
              tabIndex={0}
              className="space-y-6 mb-8 max-h-96 overflow-y-auto pr-2 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#00A3E0] focus-visible:ring-offset-2 rounded"
            >
              <div>
              ...
              <div>
                ...
                <p className="text-sm text-gray-700 leading-relaxed">
                  This assessment takes approximately 30 minutes to complete. You can pause and resume at any time. Your progress is automatically saved as you answer each question.
                </p>
              </div>
            </div>
            ...
            <div className="space-y-4 mb-8 p-4 bg-white rounded-lg border border-gray-200">
              <label className="flex items-start gap-3 cursor-pointer">
                <input
                  ...
                  className="w-5 h-5 rounded border-gray-300 text-[#00A3E0] mt-1 cursor-pointer focus-visible:ring-2 focus-visible:ring-[#00A3E0] focus-visible:ring-offset-2"
                  aria-describedby="terms-checkbox-desc"
                />
                <span id="terms-checkbox-desc" className="text-sm text-gray-700">
            ...
            <div className="flex gap-3 justify-end">
              <Link to={createPageUrl('IndividualHome')}>
                <Button variant="outline" className="border-gray-200 text-gray-700">
                  Cancel
                </Button>
              </Link>
              <Button
                onClick={() => { if (termsAccepted) setStarted(true); }}
                disabled={!termsAccepted}
                aria-disabled={!termsAccepted}
                className="bg-[#FFED00] hover:bg-[#e6d600] text-[#3D3D3D] font-semibold disabled:opacity-50 disabled:cursor-not-allowed focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-[#3D3D3D]"
              >
                Start Assessment
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-white via-gray-50/50 to-white">
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        >
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-3">
              <div
                ...
                aria-hidden="true"
              >
                <DomainIcon aria-hidden="true" className="w-5 h-5" style={{ color: question.domainColor }} />
              </div>
              <span className="font-semibold text-[#3D3D3D]">{question.domain} Domain</span>
            </div>
            <div className="flex items-center gap-2 text-sm text-gray-600" aria-live="polite" aria-atomic="true">
              <Clock aria-hidden="true" className="w-4 h-4" />
              <span>
                {Math.round(progress)}% Complete &mdash; question {currentQuestion + 1} of {SAMPLE_QUESTIONS.length}
              </span>
            </div>
          </div>
          ...
          <div className="relative">
            ...
            <div
              ...
              aria-live="polite"
              aria-atomic="true"
              className="absolute right-0 -top-8 flex items-center gap-1 text-green-700 text-sm font-medium"
              style={{ opacity: saved ? 1 : 0, transition: 'opacity 0.2s' }}
            >
        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            <div className="mb-6 pb-5 border-b border-gray-100">
              <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1">Competency</p>
              <p className="text-base font-semibold text-[#3D3D3D] leading-snug">{question.competency}</p>
            </div>

            {/* Grouping selection — radiogroup pattern */}
            <fieldset>
              <legend className="text-sm font-medium text-gray-700 mb-1">
                Choose the grouping that most closely describes your regular activities:
              </legend>
              <p id="grouping-hint" className="text-xs text-gray-600 mb-4">
                Select the level that best matches your current responsibilities
              </p>

              <div
                role="radiogroup"
                aria-labelledby="grouping-hint"
                className="space-y-3"
              >
                {question.groupings.map((grouping, index) => {
                  const isSelected = currentAnswer.grouping === grouping.id;
                  return (
                    <button
                      key={grouping.id}
                      role="radio"
                      ...
                      className={`
                        ...
                        focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#00A3E0] focus-visible:ring-offset-2
                        ${isSelected
                    ...
                    >
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          <span className={`font-semibold text-sm ${isSelected ? 'text-[#00A3E0]' : 'text-gray-700'}`}>
                            GROUPING {index + 1} — {grouping.label}
                          </span>
                          <ul className="mt-2 space-y-1" aria-label={`Grouping ${index + 1} activities`}>
                            {grouping.activities.map((activity, i) => (
                              <li key={i} className="flex items-start gap-2 text-sm text-gray-700">
                                <span aria-hidden="true" className="text-gray-500 mt-1">•</span>
                                {activity}
                              </li>
                            ))}
                          </ul>
                ...
                <button
                  role="radio"
                  aria-checked={currentAnswer.grouping === 'none'}
                  ...
                  <div className="flex items-center gap-3">
                    <div
                      ...
                      className={`
                        ...
                        ${currentAnswer.grouping === 'none' ? 'border-[#00A3E0] bg-[#00A3E0]' : 'border-gray-300'}
                      `}
                    >
                      {currentAnswer.grouping === 'none' && <div className="w-2 h-2 rounded-full bg-white" />}
                    </div>
                    <span className={`text-sm ${currentAnswer.grouping === 'none' ? 'text-[#00A3E0] font-medium' : 'text-gray-700'}`}>
        ...
        >
          <Button
            ...
            className="text-gray-700 focus-visible:ring-2 focus-visible:ring-[#00A3E0] focus-visible:ring-offset-2"
          >
          ...
          <Button
            ...
            aria-label={
              !canProceed
                ? 'Next step — please select a grouping first'
      ...
      {/* AI Chat Modal */}
      {showChat && <AIChat onClose={() => setShowChat(false)} context="assessment" />}
    </div>
  );
}