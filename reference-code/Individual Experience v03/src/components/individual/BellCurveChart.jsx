...
function getPeerCount(x, mean, std, totalN = 1200) {
...
export default function BellCurveChart({ userScore = 1.48, mean = 1.75, std = 0.42, peerLabel = 'Similar Job Level' }) {
  ...
  const toX = (x) => PL + (x - xMin) / (xMax - xMin) * chartW;
  const toY = (y) => PT + (1 - y / maxY) * chartH;
  ...
  const levelLabels = { 1.0: 'Foundational', 2.0: 'Proficient', 3.0: 'Advanced' };
  ...
  const srSummary = `Bell curve chart comparing your score to ${peerLabel}. Your score: ${userScore.toFixed(2)} out of 3, placing you at the ${userPct}th percentile. The national benchmark mean is ${mean.toFixed(2)}.`;
