...
import { Sparkles, ChevronDown, ChevronUp, TrendingUp, MessageCircle } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { base44 } from '@/api/base44Client';

export default function AISummaryCard({ prompt, title, subtitle, onChatOpen }) {
  ...
  return (
    <div
      ...
      className="bg-white border border-[#009FE8]/30 rounded-xl shadow-sm overflow-hidden cursor-pointer transition-all duration-300 hover:border-[#009FE8]/60 hover:shadow-[0_0_12px_2px_rgba(0,159,232,0.15)]"
      style={{
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <div className="flex items-center gap-2">
              <span className="text-sm font-semibold text-[#3D3D3D]">{title || 'AI Insights Summary'}</span>
              <span className="text-[10px] font-semibold uppercase tracking-widest text-[#00A3E0] bg-blue-50 px-2 py-0.5 rounded-full">AI Generated</span>
            </div>
            <p className="text-xs text-gray-400 mt-0.5">
              {expanded ? (subtitle || 'AI-generated narrative based on current data') : 'Click to generate AI insights'}
            </p>
          </div>
        </div>
